...
use crate::content::StyledContent;
...
pub(crate) struct FormattedRow(pub String);

impl FormattedRow {
    fn blank(content_width: usize) -> Self {
        FormattedRow(format!(
            "{filler}│ {blank:<content_width$}",
            filler = LineWidget::Filler,
            blank = "",
        ))
    }
}
...
pub(crate) struct LineGroup(pub Vec<FormattedRow>);
...
impl Column {
    ...
    pub fn blank(&mut self, count: usize) {
        for _ in 0..count {
            self.groups
...
///
/// ```text
/// Chrome per side: outer "│ " (2) + line widget (4) + inner "│ " (2) = 8.
...
impl ColumnPair {
    pub fn new(terminal_width: usize) -> Self {
        const CHROME: usize = 8 * 2;
        ColumnPair {
            content_width: terminal_width.saturating_sub(CHROME) / 2,
        }
    }
    ...
    pub fn zip(&self, left: Column, right: Column) -> Vec<String> {
        // Each FormattedRow is visually: widget(4) + "│ "(2) + content(content_width)
        let row_width = self.content_width + 6;
        ...
        for _ in 0..min_groups {
            ...
            for i in 0..max_rows {
                let l = left_rows.get(i).map(|r| r.0.as_str()).unwrap_or(&blank.0);
                let r = right_rows.get(i).map(|r| r.0.as_str()).unwrap_or(&blank.0);
                result.push(format!("│ {l:<row_width$}│ {r:<row_width$}"));
            }
        }
...
mod tests {
    ...
    fn highlighted(s: &str) -> Line {
        Line::new(Highlighted::new(s, Arc::new(|t: &str| format!("[hl]{t}[/]"))))
    }
    ...
    fn column_push_wraps_into_continuation_rows() {
        ...
        assert!(group[0].0.starts_with("  1 │"), "first row: {:?}", group[0].0);
        assert!(group[1].0.starts_with("  ┆ │"), "cont row: {:?}", group[1].0);
    }
    ...
    fn column_pair_zip_asymmetric_wrapping() {
        let pair = ColumnPair::new(30); // content_width = (30-16)/2 = 7
        let mut left = pair.column();
        let mut right = pair.column();
        // "hello world" at width 7 wraps to 2 rows
